...
public class DataBaseDao {
	...
	public void insertBook(String title, String author, String Genre, int option) {
		...
		String query = "insert into " + database + "." + tableName
				+ " (title, author, genre, avail, checkoutby) value (?,?,?,?,?)";
	...
	public int updateBook(int i, int found, String field) {
		if (i <= 0) {
			...
			if (i == 1) {
				String query = "UPDATE " + database + "." + tableName + " SET title = \"" + field + "\" WHERE bookId = "
						+ found + ";";
				...
				String query = "UPDATE " + database + "." + tableName + " SET author = '" + field + "' WHERE bookId = "
						+ found;
				try (Statement stmt = connection.createStatement();) {
					int rows = stmt.executeUpdate(query);
					if (rows > 0) {
				...
				String query = "UPDATE " + database + "." + tableName + " SET genre = '" + field + "' WHERE bookId = "
						+ found;
				try (Statement stmt = connection.createStatement();) {
					int rows = stmt.executeUpdate(query);
					if (rows > 0) {
				...
				String query = "UPDATE " + database + "." + tableName + " SET avail = '" + field + "' WHERE bookId = "
						+ found;
				try (Statement stmt = connection.createStatement();) {
					int rows = stmt.executeUpdate(query);
					if (rows > 0) {
	...
	public int updatefullBook(String name, String author, String genre, int available) {
		String query = "UPDATE " + database + "." + tableName + " SET name = '" + name + "', title = '"+name+"' WHERE bookId = " + author;
